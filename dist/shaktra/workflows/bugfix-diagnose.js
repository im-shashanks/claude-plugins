...
Produce: (1) the diagnosis artifact at ${a.stories_dir}/diagnosis-<bug-id>.yml; (2) a story draft with scope: bug_fix at ${a.stories_dir}/ST-<NNN>.yml (next sequential id) whose test_specs include the reproduction test that must fail before the fix; set story_path in your result. The story MUST be dev-ready — the pipeline chains straight to /shaktra:dev, which halts if the story is sparse. Choose the SMALLEST tier that fits the fix scope (most bug fixes are trivial or small — a localized fix touching <=3 files is small tier) and FULLY populate every required field for that tier per story-schema.md, including complete metadata (story_points, priority, blocked_by, status: "planned"). Do not over-tier a localized fix to medium/large. Report root cause with evidence, the fix approach, affected files, confidence, and — if the same defect pattern exists elsewhere — list each blast-radius location as an observation (type "blast-radius", text describing file and needed change).
